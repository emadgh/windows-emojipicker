...
unsafe extern "system" fn wnd_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    match msg {
        WM_NCHITTEST => {
            if let Some(hit) = BASE.drag_hit_test(hwnd, lparam, about_point_is_interactive) {
                return hit;
            }
        }
...
fn about_point_is_interactive(x: i32, y: i32) -> bool {
    ((34..346).contains(&x) && ((58..94).contains(&y) || (96..132).contains(&y)))
        || ((70..310).contains(&x) && (184..224).contains(&y))
        || ((140..240).contains(&x) && (244..278).contains(&y))
}
