...
const W: i32 = 620;
const H: i32 = 460;
const BASE: NativeWindowBase = NativeWindowBase::new(W, H, 18);
const LIST_X: i32 = 16;
const LIST_Y: i32 = 54;
...
unsafe extern "system" fn wnd_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    match msg {
        WM_NCHITTEST => {
            if let Some(hit) = BASE.drag_hit_test(hwnd, lparam, manager_point_is_interactive) {
                return hit;
            }
        }
...
fn manager_point_is_interactive(x: i32, y: i32) -> bool {
    if (LIST_X..LIST_X + LIST_W).contains(&x) && (LIST_Y..LIST_Y + LIST_H).contains(&y) {
        return true;
    }
    if (226..598).contains(&x) && (54..86).contains(&y) {
        return true;
    }
    if (226..604).contains(&x)
        && ((118..148).contains(&y) || (184..300).contains(&y) || (334..364).contains(&y))
    {
        return true;
    }
    (226..604).contains(&x) && (398..434).contains(&y)
}
...
unsafe fn handle_click(hwnd: HWND, x: i32, y: i32) -> bool {
    if (LIST_X..LIST_X + LIST_W).contains(&x) && (LIST_Y..LIST_Y + LIST_H).contains(&y) {
        ...
        if let Some(item) = custom::snapshot().get(index).cloned() {
            ...
            // Never call synchronous Win32 control APIs while a RefCell borrow is
            // alive. WM_SETTEXT can re-enter this window through color/focus
            // messages and previously caused the process to panic/terminate.
    ...
    if (226..306).contains(&x) && (398..434).contains(&y) {
        ...
        // Release ManagerState before SetWindowText/SetFocus. These APIs send
        // synchronous messages and must never execute under borrow_mut().
        set_text(title_edit, "");
